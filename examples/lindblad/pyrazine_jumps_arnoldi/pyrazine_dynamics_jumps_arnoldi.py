...
def main():
    ...
    ham = qd.Hamiltonian(H, units='ev')
    dynamics = qd.Lindblad(ham)
    results = qd.Results(tobs=tobs,e_ops=[p1,p2])
    #options = Options(method='arnoldi', nlanczos=50, unraveling=True, jump_time_finder='bisection')
    options = qd.Options(method='arnoldi', seed=1, unraveling=True, jump_time_steps=20)
    output = dynamics.solve(psi_0, times, [0.1,0.1], [qt,qc], ntraj=1, options=options, results=results)
    output.print_expectation(es_file='db_pops_lindblad_jumps_arnoldi.txt')
