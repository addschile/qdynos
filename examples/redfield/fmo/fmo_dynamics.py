...
def main():
  ...
  lamda = 35.0
  for [tau,T] in [[106.,77.], [50.,300.], [166.,300.]]:
    omega_c = 1.0/tau
    eta = 2.*lamda*omega_c
    ...
    for init in [1, 6]:
      ...
      results = qd.Results(tobs=len(times), e_ops=e_ops, print_es=True,
                   es_file='site_pops_rf_tau_%.0f_T_%.0f_init_%d.txt'%(tau,T,init))
    ...
    for init in [1, 6]:
      ...
      rho_0[init-1,init-1] = 1.0
      results = qd.Results(tobs=len(times), e_ops=e_ops, print_es=True,
                   es_file='site_pops_tcl2_tau_%.0f_T_%.0f_init_%d.txt'%(tau,T,init))
      output = dynamics.solve(rho_0, times, results=results)
