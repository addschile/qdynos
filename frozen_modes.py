...
class Frozen:
    ...
    def solve(self, rho0, times, nmodes=300, ntraj=1000, sample="Boltzmann", results=None):
        ...
        for traj in range(ntraj):
            ...
            if self.options.traj_states:
                traj_results.print_states = True
                traj_results.states_file = self.options.traj_states_file+"_traj_%d_"%(traj)
                traj_results.states_every = self.options.traj_states_every
...
# NOTE I'm only keeping this around, just in case I decide it's useful
