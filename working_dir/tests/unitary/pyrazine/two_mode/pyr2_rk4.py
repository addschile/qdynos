...
def construct_sys():
    ...
    # t
    ht1 = make_ho_h(nt,omegat,kt1,qt)
    ht2 = make_ho_h(nt,omegat,kt2,qt)
    wt1,vt1 = np.linalg.eigh(ht1)
    wt2,vt2 = np.linalg.eigh(ht2)
    ovt = np.zeros((nt,nt))
    for i in range(nt):
        for j in range(nt):
            ovt[i,j] = np.sum((vt1[:,i]*vt2[:,j])[:])
    ...
    # t2
    H += kron(phi(0),eye(nc),eye(nt))
    H += kron(phi(1),eye(nc),eye(nt))
    # Peierls coupling
    H += lamda*kron(phi(0,1),qc,ovt)
    H += lamda*kron(phi(1,0),qc,ovt.T)
...
def main():
    ...
    output = dynamics.solve(psi0, times, eig=False, options=Options(method='rk4'), results=Results(tobs=len(times), e_ops=[p1,p2], print_es=True, es_file='pyr2_rk4.txt'))
